...
function ReturnPixelHandler() {
  useEffect(() => {
    if (typeof window === "undefined") return;
    try {
      const params = new URLSearchParams(window.location.search);
      const payment_link_id =
        params.get("payment_link_id") || params.get("pl") || null;
      const order_code = params.get("order_code") || null;
      if (!payment_link_id && !order_code) return;
        ...
        try {
          const base =
            window.location.hostname === "localhost"
              ? "http://localhost:4000"
              : "";
          const q = payment_link_id
            ? `payment_link_id=${encodeURIComponent(payment_link_id)}`
            : `order_code=${encodeURIComponent(order_code)}`;
          const res = await fetch(`${base}/api/paymentlink-lookup?${q}`);
          const json = await res.json().catch(() => null);
          if (json?.ok && json.mapping) {
            const mapping = json.mapping;
            const eventId =
              (mapping.tracking && mapping.tracking.eventId) ||
              trackingLib.getEventId?.();
            // try to determine value (mapping may store amount in cents)
            let value =
              mapping.amount ||
              (mapping.custom_data && mapping.custom_data.value) ||
              null;
            if (value && typeof value === "number" && value > 1000) {
              // likely cents -> convert
              value = value / 100;
            }

            if (typeof window.fbq === "function") {
              try {
                const customData = { currency: "BRL" };
                if (value) customData.value = value;
                // send client-side pixel event with eventID for deduplication
            ...
            // cleanup short-lived event id cookie
            try {
              trackingLib.clearEventId?.();
            } catch (e) {}
          }
        } catch (e) {
          // noop
...
const Index = () => {
  ...
  const productSchema = {
    ...
    description:
      "O único rejuvenecedor celular do mundo com 31 ativos ergogênicos que nutrem os 11 sistemas do corpo simultaneamente.",
    brand: {
      "@type": "Brand",
      name: "EU+",
    },
  ...
  const organizationSchema = {
    ...
    sameAs: ["https://www.instagram.com/sejaeumais"],
    contactPoint: {
      "@type": "ContactPoint",
  ...
  return (
    <>
      <Helmet>
        ...
        <meta
          property="og:title"
          content="EU+ | O Único Rejuvenecedor Celular do Mundo"
        />
        <meta
        ...
        <meta
          property="og:image"
          content="https://sejaeumais.com/og-image.jpg"
        />
        ...
        <meta
          name="twitter:title"
          content="EU+ | Rejuvenecedor Celular Patenteado"
        />
        <meta
